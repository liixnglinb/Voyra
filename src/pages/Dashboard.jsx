...
function FeatureArt({ type }) {
  ...
  if (type === 'tokenmonitor') {
    ...
    const svc = [["zcode", "1.17 B", "#5B8DEF", "Z", 100], ["claude-code", "696.8 M", "#9B8CFF", "C", 59], ["codex", "641.8 M", "#4FC3A1", "X", 55]];
    return <div ref={artRef} onPointerEnter={() => { pausedRef.current = true; }} onPointerLeave={() => { pausedRef.current = false; }} className="vr-art vr-tool-art vr-tm-art">
      ...
      <div className="vr-tm-kpi">
        <div className={active === 0 ? 'is-on' : ''}><span>TOKENS</span><b>3.37 B</b><em>缓存命中 83.1%</em></div>
        <div className={active === 1 ? 'is-on' : ''}><span>API 请求</span><b>12,092</b><em>逐条计价</em></div>
        <div className={active === 2 ? 'is-on' : ''}><span>估算成本</span><b>&#165;11,012</b><em>按模型单价</em></div>
      </div>
