...
export default function Dashboard() {
  ...
  return <div className="vr-home" ref={rootRef}>
    ...
    <style>{`
      ...
      .vr-home .vr-person-stage::before { content: ""; position: absolute; top: 13%; right: -15%; width: 86%; aspect-ratio: 1; border: 1px solid rgba(27,27,27,.1); border-radius: 50%; background: rgba(255,255,255,.24); }
    ...
    <main><section className="vr-hero-shell"><div className="vr-hero" data-roll><div className="vr-hero-copy"><h1><span>Voyra</span><span>makes</span><span className="vr-hero-outline">ideas</span><span>useful.</span></h1><div className="vr-hero-meta"><strong>帅帅你阿历</strong><span>PERSONAL TOOLS / AI / OPEN-SOURCE</span></div><div className="vr-scroll-cue"><ChevronDown size={16} /> 向下探索</div></div><div className="vr-person-stage" aria-hidden="true"><div className="vr-person-frame"><div className="vr-person-motion"><img className="vr-person-skin" src="/hero/voyra-person-skin-v3.png" alt="" /><img className="vr-person-body" src="/hero/voyra-person-body-v2.png" alt="" /><img className="vr-person-hair" src="/hero/voyra-person-hair-v2.png" alt="" /><img className="vr-person-collar" src="/hero/voyra-person-collar-v2.png" alt="" /></div></div></div></div></section><section className="vr-stage vr-tab-zone" data-active-work={activeTab} aria-label="内容分类"><TabReel activeTab={activeTab} /><div className="vr-tabs" data-roll role="tablist" aria-label="内容分类">{TABS.map(([id, label]) => <button id={`work-tab-${id}`} key={id} role="tab" aria-controls={`panel-${id}`} aria-selected={activeTab === id} className={`vr-tab${activeTab === id ? ' is-active' : ''}`} onClick={() => changeTab(id)}>{label}</button>)}</div><div className="vr-panels">{TABS.map(([id, label]) => <div className="vr-panel" ref={(node) => { panelRefs.current[id] = node; }} id={`panel-${id}`} role="tabpanel" aria-labelledby={`work-tab-${id}`} aria-label={label} aria-hidden={activeTab !== id} hidden={activeTab !== id} key={id}>{panels[id]}</div>)}</div></section></main>
