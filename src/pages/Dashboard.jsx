  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      root.querySelectorAll('[data-roll]').forEach((node) => {
        ...
        const entering = clamp((rect.top - viewport.top - viewport.height * 0.18) / (viewport.height * 0.72), 0, 1);
        const leaving = clamp((viewport.top + viewport.height * 0.14 - rect.bottom) / (rect.height + viewport.height * 0.2), 0, 1);
        node.style.setProperty('--roll-y', `${(entering * 36) - (leaving * 12)}px`);
        node.style.setProperty('--roll-angle', `${(-entering * 7) + (leaving * 3)}deg`);
        node.style.setProperty('--roll-opacity', `${1 - Math.max(entering * 0.32, leaving * 0.08)}`);
      });
      frame = 0;
    };
    const requestUpdate = () => {
...
function ProductPanel({ go }) {
  ...
  return <div className="vr-product-list">{FEATURED.map((feature, index) => {
    ...
    return (
      <div className="vr-roll-wrap" data-roll key={feature.to}><article className={`vr-feature vr-card${index % 2 ? ' is-reverse' : ''}`} data-reveal style={{ '--reveal-delay': `${Math.min(index * 0.08, 0.28)}s` }} onPointerMove={updateSpotlight}>
...
function SkillsPanel() {
  return <section className="vr-skill-group vr-solo-skill" data-roll>
...
function ArticlesPanel({ go }) {
  return <div className="vr-article-grid">{ARTICLES.map((article, index) => <div className="vr-roll-wrap" data-roll key={article.title}><button className="vr-article-card vr-card" data-reveal style={{ '--reveal-delay': `${Math.min(index * 0.08, 0.24)}s` }} onPointerMove={updateSpotlight} onClick={() => go('/blog')}>
...
function ContactPanel() {
  return <section className="vr-contact-panel"><div className="vr-contact-head" data-reveal><span>联系 / ELSEWHERE</span><b>保持交流</b></div><div className="vr-contact-list">{CONTACTS.map((contact, index) => {
...
export default function Dashboard() {
  ...
  return <div className="vr-home" ref={rootRef}>
    ...
    `}</style>
    <style>{`
      .vr-home { background-image: radial-gradient(80% 50% at 50% -10%, rgba(255, 226, 138, .32), transparent 70%), radial-gradient(72% 62% at 0% 62%, rgba(255, 232, 184, .2), transparent 73%), linear-gradient(rgba(29,29,29,.055) 1px, transparent 1px), linear-gradient(90deg, rgba(29,29,29,.055) 1px, transparent 1px); background-size: auto, auto, 32px 32px, 32px 32px; background-attachment: fixed; }
      .vr-home [data-roll] { --roll-y: 0px; --roll-angle: 0deg; --roll-opacity: 1; min-width: 0; opacity: var(--roll-opacity); transform: perspective(900px) translate3d(0, var(--roll-y), 0) rotateX(var(--roll-angle)); transform-origin: 50% 50%; transform-style: preserve-3d; transition: opacity .18s linear; will-change: transform, opacity; }
      ...
      .vr-home .vr-top { width: min(100% - 48px, 1056px); }
      .vr-home .vr-hero-shell { width: min(100% - 48px, 1056px); min-height: calc(100svh - 34px); margin: 0 auto; display: grid; grid-template-columns: minmax(0, 1fr) minmax(280px, .72fr); gap: 44px; align-items: stretch; }
      .vr-home .vr-hero { min-height: 0; padding: 58px 0 60px; justify-content: center; }
      .vr-home .vr-hero h1 { max-width: 680px; font-size: 112px; font-weight: 780; line-height: .89; }
      .vr-home .vr-hero h1 span:nth-child(4) { animation-delay: .21s; }
      .vr-home .vr-hero-outline { color: transparent; -webkit-text-stroke: 1.5px var(--ink); }
      .vr-home .vr-hero-meta { margin-top: 34px; }
      .vr-home .vr-hero-visual { position: relative; min-height: 100%; }
      .vr-home .vr-hero-visual::before { content: ""; position: absolute; top: 18%; bottom: 18%; left: 50%; width: 1px; background: var(--line); }
      .vr-home .vr-hero-visual::after { content: ""; position: absolute; top: 18%; left: 50%; width: 78px; height: 78px; border-top: 1px solid var(--line); border-right: 1px solid var(--line); }
      ...
      @media (max-width: 720px) { .vr-home .vr-top { width: min(100% - 40px, 756px); } .vr-home .vr-hero-shell { width: min(100% - 40px, 756px); min-height: calc(100svh - 14px); display: block; } .vr-home .vr-hero { min-height: calc(100svh - 14px); padding: 78px 0 54px; } .vr-home .vr-hero h1 { font-size: 62px; line-height: .91; } .vr-home .vr-hero-meta { margin-top: 30px; } .vr-home .vr-hero-visual { display: none; } .vr-home .vr-tab { font-size: 28px; } .vr-home [data-roll] { transform: translate3d(0, var(--roll-y), 0); } .vr-home [data-reveal] { transform: translate3d(0, 26px, 0); filter: blur(4px); } .vr-home [data-reveal].vr-is-visible { transform: translate3d(0, 0, 0); } }
    ...
    <main><section className="vr-hero-shell"><div className="vr-hero"><h1><span>Voyra</span><span>makes</span><span className="vr-hero-outline">ideas</span><span>useful.</span></h1><div className="vr-hero-meta"><strong>帅帅你阿历</strong><span>PERSONAL TOOLS / AI / OPEN-SOURCE</span></div><div className="vr-scroll-cue"><ChevronDown size={16} /> 向下探索</div></div><div className="vr-hero-visual" aria-hidden="true" /></section><section className="vr-stage vr-tab-zone" aria-label="内容分类"><div className="vr-tabs" role="tablist" aria-label="内容分类">{TABS.map(([id, label]) => <button key={id} role="tab" aria-selected={activeTab === id} className={`vr-tab${activeTab === id ? ' is-active' : ''}`} onClick={() => changeTab(id)}>{label}</button>)}</div><div className="vr-panel" role="tabpanel" aria-label={TABS.find(([id]) => id === activeTab)?.[1]} key={activeTab}>{panel}</div></section></main>
